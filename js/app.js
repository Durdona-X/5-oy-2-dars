...
elThemes.addEventListener("click", () => {
  const currentTheme = document.documentElement.dataset.theme;

  if (currentTheme === "dark") {
    document.documentElement.dataset.theme = "light";
    elThemes.innerText = "Dark mode";
  } else {
    document.documentElement.dataset.theme = "dark";
    elThemes.innerText = "Light mode";
  }
});
...
elContainer.addEventListener("click", (evt) => {
  ...
  // GET
  if (
    evt.target.classList.contains("card-body") &&
    !evt.target.classList.contains("js-delete-button")
  ) {
    window.open(window.location.origin + "/details.html", "_blank");
  }
});
...
//   name: elCarName.value,
//   year: elCarYear.value,
//   color: elCarColor.querySelector("input").value,
//   power: elCarPower.querySelector("input").value,
// };
...
function getById(id) {
  fetch("https://json-api.uz/api/project/fn44-amaliyot/cars/${id}")
    ...
    .then((res) => {
      ...
      elEditForm.name.defaultValue = res.name;
      elEditForm.year.defaultValue = res.year;
      elEditForm.color.value = res.color;
      elEditForm.maxSpeed.defaultValue = res.maxSpeed;
      elEditForm.horsepower.defaultValue = res.horsepower;
    })
    .catch(() => {})
    .finally(() => {});
}
